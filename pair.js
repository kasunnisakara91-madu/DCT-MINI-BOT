...
const BOT_NAME_FANCY = 'DCT MINI BOT';
...
const config = {
  ...
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/ERtSi7im78wCBNjiSddoLB?s=cl&p=a&ilr=4&amv=2',
  RCD_IMAGE_PATH: 'https://files.catbox.moe/k8zmqd.jpeg',
  NEWSLETTER_JID: [
      '120363421785026867@newsletter'],
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER ? process.env.OWNER_NUMBER.split(',') : ['94787940686','94752978237'],
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb7Lf8I9sBI8QXTIZv1P',
  BOT_NAME: '© DCT MINI BOT',
  BOT_VERSION: '1.0.0 ULTRA',
  OWNER_NAME: 'MADUSANKA,DULA DEV,KEZU',
  IMAGE_PATH: 'https://files.catbox.moe/k8zmqd.jpeg',
  BOT_FOOTER: '> *© DCT MINI BOT*',
...
✨ ༺ DCT MINI BOT 𝗦𝘆𝘀𝘁𝗲𝗺 𝗶𝘀 𝗻𝗼𝘄 𝗼𝗻𝗹𝗶𝗻𝗲 & 𝗿𝗲𝗮𝗱𝘆!`,
  useBotName
);
      ...
      case 'tourl':
        ...
        case 'upload': {
          ...
          const metaQuote = {
            ...
            message: { contactMessage: { displayName: "DCT MINI BOT", vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:Upload Service\nORG:Catbox/ImgBB\nEND:VCARD` } }
          };
...
`.trim();
  ...
  break;
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '© DCT MINI BOT ||🍃'; // dynamic bot name
...
`
    ...
    break;
          ...
          try {
            ...
            const botName = cfg.botName || "DCT MINI BOT";
            const logo = cfg.logo || config.RCD_IMAGE_PATH;
...
case 'csong': {
    try {
        ...
        try {
            ...
            const _0x6368616d61 = "Powered by ༺ DCT MINI BOT"; 
...
case 'alive': {
        ...
        case 'owner': {
          ...
          // ✅ BOT NAME
          const BOT_NAME = "DCT MINI BOT";

          // ✅ OWNER DETAILS
          const ownerName = "DCT MINI BOT";
          const ownerNumber = "94783731694"; // without +
          const displayNumber = "+94 78 373 1694";
          const email = "dctminibot@email.com"; // optional
...
> Channel data retrieved successfully`;
        ...
        case 'follow': {
          try {
            ...
            if (!channelLink || !channelLink.trim()) {
              return await socket.sendMessage(sender, {
                text: `❌ *Channel Link Required!*\n\n📝 Usage: .channelfollow <channel_link>\n\n🔗 *Examples:*\n• .channelfollow https://whatsapp.com/channel/0029Vb7p3UCCHDyocfEGm23f\n• .channelfollow 120363423916773660@newsletter\n• .channelfollow https://chat.whatsapp.com/channel/0029VbBivQGBKfi1VaWyEd0t`
              }, { quoted: msg });
            }
...
case 'video': {
  ...
  try {
    ...
    const caption = `*🎬 DCT MINI BOT 𝗩ɪᴅᴇᴏ 𝗗ᴏᴡɴʟᴏᴀᴅᴇʀ ??*
